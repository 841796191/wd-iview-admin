  ...
  <div>
    ...
    <Row :gutter="20" style="margin-top: 10px;">
      <i-col :md="24" :lg="8" style="margin-bottom: 20px;">
        <Card shadow>
          <chart-pie :key="timer2" tipname="统计内容" style="height: 300px;" :value="pieData" text="发帖统计"></chart-pie>
        </Card>
      </i-col>
      <i-col :md="24" :lg="16" style="margin-bottom: 20px;">
        <Card shadow>
          <chart-bar :key="timer3" style="height: 300px;" :value="barData" text="近6月累计发帖"/>
        </Card>
      </i-col>
    </Row>
    <Row :gutter="20">
      <Card shadow>
        <week-stat :key="timer4" :weekData="weekData" style="height: 310px"></week-stat>
      </Card>
    </Row>
  </div>
</template>
  ...
  data () {
    return {
      ...
      inforCardData: [
        ...
        { title: '本周采纳', icon: 'md-checkmark-circle', count: 0, color: '#ed3f14' },
      ...
      getStatData().then((res) => {
        if (res.code === 200) {
          if (res.data.inforCardData) {
            ...
            // 为data里面的对象赋完值可以通过为组件绑定key,然后更新那个key的方式使数据更新
          ...
          if (res.data.monthData) {
            ...
            // 补足未有月份
            for (let i = 0; i <= 5; i++) {
              const key = moment().subtract(6 - i, 'M').format('YYYY-MM')
