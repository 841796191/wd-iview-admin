...
export default {
  ...
  data () {
    return {
      isEdit: false, // 控制角色列表和菜单权限按钮显示与否
      showAdd: false,
      modelEdit: false, // 修改角色
      editIndex: '',
      loading: true,
      roles: [],
      roleIndex: '',
  ...
  methods: {
    ...
    // 点击角色,给菜单和资源选项添加checked状态,通过this.roles中的menu记录角色拥有什么权限,menu记录了权限的_id
    selectRole (value) {
      debugger
      if (this.roleIndex === '' || this.roleIndex !== value) {
        ...
        // modifyNode用于给表单添加check状态,根据roles的menu来确定
        // 修改右侧菜单树 + 权限列表的选中状态
    ...
    submit () {
      this.isEdit = false
      localStorage.setItem('menuData', JSON.stringify(this.menuData))
      // 提取menuData中的children、operations属性
    ...
    handleTreeChecked (item) {
      // this.isEdit判断有没有点击修改按钮
      if (!this.isEdit) {
        // 取得角色信息初始值
        const tmpData = localStorage.getItem('menuData')
        // 如果temData有值且没有点选修改按钮则赋回初始值
        if (typeof tmpData !== 'undefined') {
          this.menuData = JSON.parse(tmpData)
        }
